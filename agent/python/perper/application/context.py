...
def run_init_delegate(function: Callable, use_deploy_init=False):
    if use_deploy_init:
        init_instance: str = (
            ...
            if startup_context.get().instance is not None
            else f"{startup_context.get().agent}-Init"
        )
        ...
        startup_context.get().task_collection.add(
            process_execution(init_execution, function, is_init=True)
        )
    else:
    ...
    async def helper():
        if is_init:
            async for execution in fabric_service.get().listen_executions(
                ...
                if (
                    startup_context.get().instance is None
                    or startup_context.get().instance == execution.execution
                ):
                    ...
                    startup_context.get().task_collection.add(
                        process_execution(transformed_execution, function, is_init)
                    )
        else:
            async for execution in fabric_service.get().listen_executions(
                PerperInstance(
                ...
                startup_context.get().task_collection.add(
                    process_execution(execution, function)
                )
...
async def process_execution(
    execution: FabricExecution, function: Callable, is_init=False
):
    try:
        ...
        if not is_init:
            if result is None:
                await fabric_service.get().write_execution_finished(
                    fabric_execution.get().execution
                )
                ...
                async for data in result:
                    if (
                        ...
                        (key, data) = data
                        await fabric_service.get().write_stream_item(
                            fabric_execution.get().execution, key, data
                        )
                    else:
                        await fabric_service.get().write_stream_item_realtime(
                            fabric_execution.get().execution, data
                        )
                await fabric_service.get().write_execution_finished(
                    fabric_execution.get().execution
                )
            else:
                await fabric_service.get().write_execution_result(
                    fabric_execution.get().execution, result
                )
        ...
        if not is_init:
            try:
                await fabric_service.get().write_execution_exception(
                    fabric_execution.get().execution, ex
                )
            except Exception as ex:
                print(f"Error while executing {fabric_execution.get().execution}:")
                traceback.print_exception(type(ex), ex, ex.__traceback__)
