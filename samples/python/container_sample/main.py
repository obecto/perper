...
async def main():
    ...
    (k, n) = await get_notification_service().get_notification(
        get_notification_service().CALL, "Startup"
    )
    ...
    async for (k, n) in get_notification_service().get_notifications(
        get_notification_service().CALL, "Test"
    ):
