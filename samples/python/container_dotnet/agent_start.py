...
async def main(agent):
    ...
    startup_execution = await task_to_future(lambda ct: fabric_service.GetExecutionsReader(agent, instance, "Startup").ReadAsync(ct))
    startup_parameters = await task_to_future(lambda _: fabric_service.ReadExecutionParameters(startup_execution.Execution))
    ...
    async for test_execution in convert_async_iterable(fabric_service.GetExecutionsReader(agent, instance, "Test").ReadAllAsync().GetAsyncEnumerator()):
