...
def boxes_execution_plan(workers, config):
...
#       Changing buy rewards postfaktum
        for step in range(len(batch['infos'])):
            action_type = batch['infos'][step]['action_type']
            buy_rewards_info = batch['infos'][step]['buy_rewards_info']
            
            if buy_rewards_info:
                for timestep, buy_reward in buy_rewards_info:
                    batch['rewards'][timestep] += buy_reward
...
Boxes_DDPPOTrainer = DDPPOTrainer.with_updates(
    execution_plan = boxes_execution_plan)