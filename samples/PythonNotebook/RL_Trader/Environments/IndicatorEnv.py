...
from Data.random_indicator_load import random_train_load
import logging
...
class SimpleMarket(gym.Env):
    def __init__(self, env_config = {'action_mode': 1,
        ...
        self.high = load('Data/Indicator_based_RL/high_values.npy')[1:]
        self.high = np.append(self.high, [100., 2000., 1000., self.high[0]])
        
        self.low = load('Data/Indicator_based_RL/low_values.npy')[1:]
        self.low = np.append(self.low, [0., 0., 0., self.low[0]])
        #print(len(self.low))
        ...
        self.observation_space = spaces.Box(high = np.inf, low = -np.inf, shape = (263,))
#         logger.debug("Version: {}  shape: {}   Actions: {}".format(self.version, low.shape, self.action_space))
    ...
    def reset(self, ):
        ...
        self.data = random_train_load('Data/Indicator_based_RL/indicator_dataset.csv', ep_size = self.episode_length)
        self.tick = 0 
    ...
    def test(self, ):
        ...
        self.data = pd.read_csv('Data/Label_based_RL/test_data_rl.csv')
        self.tick = 0 
        state = self.data.iloc[self.tick]
        #state = np.append(state[1:], [100, self.current_money, 0])
        #CHANGE NEXT DUMMY ACTION TO ZERO
    ...
    def normalize_state(self, state):
        normalized_state = (state - self.low)/((self.high - self.low) + 1e-15) 
...
#           Tracking the overall movement of the reward
        ...
        self.current_price = next_state[0]
